...
async function seed() {
  const db = getDb();
  console.log("Seeding Kimi's Horizon database...");
  ...
  await db.insert(schema.sessions).values([
    {
      worldId,
      sessionId: 1,
      title: "Arrival at New Horizon Island",
    ...
    },
    {
      worldId,
      sessionId: 8,
      title: "Final Broadcast and Reflection",
  ...
  await db.insert(schema.events).values({
    ...
    sessionId: 1,
    sectorId: "all",
    eventTitle: "First Council Meeting",
    eventDetails: "The World Council asks all teams to investigate their sector and report back.",
    active: "true",
  });
